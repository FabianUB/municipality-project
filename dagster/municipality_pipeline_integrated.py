"""
Integrated municipality analytics pipeline with dbt - single file version
"""
import os
import shutil
import glob
...
from dagster import asset, job, Definitions, Output, AssetExecutionContext
...
def municipality_dbt_models(context: AssetExecutionContext) -> Output[dict]:
    """Execute all dbt models by creating a trigger file for the dbt container"""
    import os
    import time
    
    context.log.info("Triggering dbt build process via file system")
    
    try:
        # Create a trigger file to signal the dbt container to run dbt build
        trigger_file = "/opt/dagster/dbt/run_dbt_build.trigger"
        result_file = "/opt/dagster/dbt/dbt_build_result.txt"
        ...
        # Wait for the dbt container to process the request (check for result file)
        ...
        while waited_time < max_wait_time:
            ...
            context.log.info(f"Waiting for dbt build to complete... ({waited_time}s)")
        
        if not os.path.exists(result_file):
            context.log.error("dbt build timed out - no result file found")
            return Output(
                {
                    "status": "timeout",
                    "message": "dbt build timed out after 5 minutes"
                },
                metadata={
        ...
        context.log.info(f"dbt result: {result_content}")
...
def demography_etl_pipeline():
    """ETL pipeline: Excel → CSV → PostgreSQL"""
